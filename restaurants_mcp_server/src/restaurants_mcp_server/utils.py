import restaurants_mcp_server.instructor_compat  # noqa: F401 — Superlinked NLQ vs instructor 1.14+ API

import os

from superlinked import framework as sl

from restaurants_mcp_server.qdrant_url import resolve_qdrant_url
from restaurants_mcp_server.superlinked_app.index import business, business_index
from restaurants_mcp_server.superlinked_app.query import query
from restaurants_mcp_server.superlinked_app.utils import format_minute_columns_to_hhmm
...
def process_context(context):
    formatted_context = ""
    for id, name, rating, review_count, state, city, similarity_score in zip(
        context["retrived_restaurant_ids"],
        context["retrived_restaurants_names"],
        context["retrived_restaurants_ratings"],
        context["retrived_restaurants_reviews_count"],
        context["retrived_states"],
        context["retrived_cities"],
        context["similarity_scores"],
    ):
        formatted_context += (
            f"-ID: {id}, Name: {name}, Rating: {rating}, Review Count: {review_count}, "
            f"State: {state}, City: {city}, Similarity Score: {similarity_score}\n"
        )
